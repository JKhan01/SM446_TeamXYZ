...
class bitbucketActions():
    ...
    def get_commit_by_user(self,repo_name,owner_name,user_name):
        ...
        if resp.status_code == 200:
            ...
            returnVar = {}
            for i in out['values']:
                if str(user_name).casefold() in i['author']['user']['display_name']:
                    returnVar['author'] = str(i['author']['user']['display_name'])
                    returnVar['message'] = str(i['message'])
                    returnVar['date'] = str(i['date'][:20]).replace('T',' ')
                    returnVar['pagelink'] = str(i['links']['html']['href'])
                    sCode = requests.get(self.baseUrl+'repositories/'+str(owner_name)+'/'+str(repo_name)+'/src/'+i['hash']+'/')
                    if sCode.status_code == 200:
                        returnVar['codeLinks'] = [j['links']['self']['href'] for j in sCode.json()['values']]
                    else:
                        returnVar['codeLinks'] = []
                    break
            self.returnJson = returnVar
        else:
